...
class CallkitIncomingBroadcastReceiver : BroadcastReceiver() {
        ...
        when (action) {
            "${context.packageName}.${CallkitConstants.ACTION_CALL_INCOMING}" -> {
                try {
                    ...
                    if (fromUi) {
                        // Custom: show incoming directly from UI (no notification/sound)
                        sendEventFlutter(CallkitConstants.ACTION_CALL_INCOMING, data)
                        addCall(context, Data.fromBundle(data))
                        if (notificationtime != null) {
                            var durationleft = 30L - Duration.between(notificationtime, Instant.now()).seconds
                            if (durationleft < 0) durationleft = 0
                            data.putLong(CallkitConstants.EXTRA_CALLKIT_DURATION, durationleft)
                            data.putLong("duration", durationleft)
                        }
            ...
            "${context.packageName}.${CallkitConstants.ACTION_CALL_TIMEOUT}" -> {
                try {
                    getCallkitNotificationManager()?.clearIncomingNotification(data, false)
                    getCallkitNotificationManager()?.showMissCallNotification(data)
    ...
    private fun sendEventFlutter(event: String, data: Bundle) {
        ...
        val missedCallNotification = mapOf(
            "id" to data.getInt(CallkitConstants.EXTRA_CALLKIT_MISSED_CALL_ID, 0),
            "showNotification" to data.getBoolean(CallkitConstants.EXTRA_CALLKIT_MISSED_CALL_SHOW, true),
            "count" to data.getInt(CallkitConstants.EXTRA_CALLKIT_MISSED_CALL_COUNT, 1),
            ...
            "isShowCallback" to data.getBoolean(CallkitConstants.EXTRA_CALLKIT_MISSED_CALL_CALLBACK_SHOW, true),
        )
        val callingNotification = mapOf(
            "id" to data.getString(CallkitConstants.EXTRA_CALLKIT_CALLING_ID, ""),
            "showNotification" to data.getBoolean(CallkitConstants.EXTRA_CALLKIT_CALLING_SHOW, true),
            ...
            "isShowCallback" to data.getBoolean(CallkitConstants.EXTRA_CALLKIT_CALLING_HANG_UP_SHOW, true),
        )
