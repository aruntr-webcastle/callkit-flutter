...
class FlutterCallkitIncomingPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    companion object {
        ...
        fun getInstance(): FlutterCallkitIncomingPlugin? {
            return if (hasInstance()) instance else null
        }
    ...
    fun getCallkitNotificationManager(): CallkitNotificationManager? = callkitNotificationManager
    fun getCallkitSoundPlayerManager(): CallkitSoundPlayerManager? = callkitSoundPlayerManager
    ...
    public fun showIncomingNotification(data: Data) {
        ...
        context?.sendBroadcast(
            CallkitIncomingBroadcastReceiver.getIntentIncoming(requireNotNull(context), data.toBundle())
        )
    }
    ...
    public fun startCall(data: Data) {
        context?.sendBroadcast(
            CallkitIncomingBroadcastReceiver.getIntentStart(requireNotNull(context), data.toBundle())
        )
    }

    public fun endCall(data: Data) {
        context?.sendBroadcast(
            CallkitIncomingBroadcastReceiver.getIntentEnded(requireNotNull(context), data.toBundle())
        )
    }
    ...
    public fun endAllCalls() {
        ...
        calls.forEach {
            context?.sendBroadcast(
                CallkitIncomingBroadcastReceiver.getIntentEnded(requireNotNull(context), it.toBundle())
            )
        }
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        try {
            when (call.method) {
                ...
                // Custom: show incoming directly from UI (no notification/sound, open activity with fromUi)
                "showCallkitIncomingDirect" -> {
                    ...
                    context?.sendBroadcast(
                        CallkitIncomingBroadcastReceiver.getIntentIncoming(
                            requireNotNull(context),
                            data.toBundle().apply { putBoolean("fromUi", true) }
                        )
                    )
                    result.success(true)
                }
                ...
                "muteCall" -> {
                    val map = buildMap {
                        ...
                        if (args is Map<*, *>) putAll(args as Map<String, Any>)
                    }
                ...
                "activeCalls" -> result.success(getDataActiveCallsForFlutter(context))
                "getDevicePushTokenVoIP" -> result.success("")
                "silenceEvents" -> {
                    CallkitIncomingBroadcastReceiver.silenceEvents = call.arguments as? Boolean ?: false
                    result.success(true)
                }
                ...
                "endNativeSubsystemOnly" -> result.success(true)
                "setAudioRoute" -> result.success(true)
            }
